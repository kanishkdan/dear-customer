...
## 1.0.5 (the store already holds an earlier 1.0.4 build; this supersedes it)
...
- WhatsApp's own opt-out is only planned for senders WhatsApp itself tagged as marketing (marketing template or marketing-thread flag). For a keyword-guessed promo sender it shows "Not offered" with the reason, and a server "not-acceptable" answer is also "Not offered". Neither counts as a failure, so a business no longer reads "Partly completed" because of it. A mixed number with no promotions-only way out gets no STOP planned (nothing "skipped").
- Built as `dist/dear-customer-1.0.5.zip`.
