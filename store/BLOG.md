...
So I built Bouncer, a Chrome extension for WhatsApp Web. It reads your chats, finds every business that has been sending you promotions, and shows how many numbers each one has burned on you. Tick them, press Bounce, and for each number it:
...
Your bank's OTPs and your grocer's delivery updates stay untouched. WhatsApp stamps every template with the category the business declared, marketing or utility or authentication, and Bouncer reads that. It also doesn't trust it blindly, because plenty of "utility" templates in India say "pre-approved loan, apply now". Words and call-to-action buttons outrank a utility tag.
...
Two honest caveats. Your phone still buzzes once per new number, because phone and browser receive the message at the same instant and nothing in a browser can prevent that. And this automates WhatsApp Web, which is against their terms. Bans in practice hit bulk senders and Bouncer sends at most one message per business with pauses between, but it isn't zero risk.
...
Get it: bouncer.kanishkdan.com. Source: github.com/kanishkdan/bouncer.
